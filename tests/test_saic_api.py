...
class TestSaicApi(IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        async with ClientSession() as client_session:
            self.saic_api = SaicApi('https://tap-eu.soimt.com', 'user@home.de', 'secret', client_session)
    ...
    async def test_login(self, mocked_post):
    ...
    @patch.object(ClientSession, 'post')
    async def test_set_alarm_switches(self, mocked_post):
        ...
        try:
            ...
            await self.saic_api.set_alarm_switches(alarm_switches)
        except SaicApiException:
            self.fail()

    @patch.object(ClientSession, 'post')
    async def test_get_vehicle_status(self, mocked_post):
        ...
        vehicle_status_rsp_msg = await self.saic_api.get_vehicle_status_with_retry(vin_info)
    ...
    @patch.object(ClientSession, 'post')
    async def test_get_charging_status(self, mocked_post):
        vin_info = create_vin_info(VIN)
        mock_response(mocked_post, mock_chrg_mgmt_data_rsp(self.message_coder_v3_0, UID, TOKEN, vin_info))

        chrg_mgmt_data_rsp_msg = await self.saic_api.get_charging_status_with_retry(vin_info)
    ...
    @patch.object(ClientSession, 'post')
    async def test_start_ac(self, mocked_post):
        vin_info = create_vin_info(VIN)
        mock_response(mocked_post, mock_start_ac_rsp_msg(self.message_coder_v2_1, UID, TOKEN, vin_info))

        start_ac_rsp_msg = await self.saic_api.start_ac(vin_info)
